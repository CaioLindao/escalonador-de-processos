// -====================================================================================- 
// TRABALHO EM EQUIPE - ESCALONAMENTO DE PROCESSOS (SISTEMAS OPERACIONAIS - S3 - 2022.1)
// EQUIPE:
// Caio de Almeida Araujo
// Luan Alves Fernandes
...
struct Process
{
    ...
    void show()
    {
        cout << pID << " | arrival:  " << arrival << " | burst: " << burst << " | prio: " << priority << " | ready: " << ready << " | start: " << start << " | turnaround: " << turnaround << " | exec: " << executedTime << " | term: " << terminated << " | last: " << lastQueued << " | context: " << contextChanges << " | quantumTimes: " << quantumTimes << endl;
        system("pause");
    }
};
...
// -=================================================-
// FUNCOES PRINCIPAIS (FCFS, SRTF, SRTFP, PRIO, PRIOP)
// -=================================================-
...
vector<Process> shortestRemainingTimeFirst(vector<Process> processes, int queueTime)
{
    ...
    if (arrieved.size())
    {
        ...
        if (processes[index].lastQueued != (queueTime - 1) && processes[index].lastQueued != 0)
        {
            processes[index].contextChanges++;
        }
        ...
        // atualiza o lastQueued
        processes[index].lastQueued = queueTime;
    }
...
vector<Process> roundRobin(vector<Process> processes, int queueTime)
{
    ...
    do
    {
        ...
        if (arrieved.size())
        {
            ...
            if (processes[index].quantumTimes == 4)
            {
                arrieved.erase(arrieved.begin());
                arrieved.push_back(processes[index]);

                processes[index].quantumTimes = 0;
                index = arrieved[0].pID - 1;
            }
    ...
    else
    {
        ...
        if (csv.is_open())
        {
            ...
            if (modoEscalonamento == "FCFS" && modoExecucao == "1")
            {
                ...
                cout << "PIDS | CHEGADA | RAJADA | TERMINO" << endl;
                for (int i = 0; i < processes.size(); i++)
                {
                    cout << processes[i].pID << " | " << processes[i].arrival << " | " << processes[i].burst << " | " << processes[i].ready << endl;
                }
            };
            ...
            {
                cout << "PID | TEMPO DE FINALIZADO" << endl;
                ...
                for (int i = 0; i < processes.size(); i++)
                {
                    cout << processes[i].pID << " - " << processes[i].ready << endl;
                }
            };
            ...
            if (modoEscalonamento == "SRTF" && modoExecucao == "2")
            {
                ...
                cout << "tempo total de espera     = " << (queueTime - 2) << endl;
                cout << "media do tempo de retorno = " << (totalTurnaroundTime / processes.size()) << endl;
                cout << "media do tempo de espera  = " << (totalWaitTime / processes.size()) << endl;
                cout << "quantidade de trocas de contexto  = " << (totalContextChanges) << endl;
            };
            ...
            if (modoEscalonamento == "RR" && modoExecucao == "2")
            {
                ...
                cout << "tempo total de espera     = " << (processes[processes.size()-1].ready) << endl;
                cout << "media do tempo de retorno = " << (totalTurnaroundTime / processes.size()) << endl;
                cout << "media do tempo de espera  = " << (totalWaitTime / processes.size()) << endl;
                cout << "quantidade de trocas de contexto  = " << (totalContextChanges) << endl;
